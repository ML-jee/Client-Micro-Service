...
public class UserAuthenticationProvider {
    ...
    public String createToken(UserDto client) {
        Date now = new Date();
        Date validity = new Date(now.getTime() + 360000000); // 1 hour

        Algorithm algorithm = Algorithm.HMAC256(secretKey);
        return JWT.create()
                .withSubject(client.getAdresseWallet())
                .withIssuedAt(now)
    ...
    public Authentication validateTokenStrongly(String token) {
        ...
        UserDto client = userService.findByAdresseWallet(decoded.getSubject());
